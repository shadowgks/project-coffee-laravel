    ...
    <section>
        ...
        <div>
            ...
            <div class="border-b coffee-bg">
                <ul class="flex justify-center flex-wrap -mb-px text-sm font-medium text-center" id="myTab" data-tabs-toggle="#myTabContent">
                    <li class="mr-2" role="presentation">
                        <a class="inline-block p-4 border-b-2 rounded-t-lg" id="all-tab" data-tabs-target="#all" type="button" role="tab" aria-controls="all">All</a>
                    </li>
                    @foreach ($recent_plates['categories'] as $item)
                        <li class="mr-2" role="presentation">
                            <button class="inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600" data-tabs-target="#{{ $item->name }}" type="button" role="tab">{{ $item->name }}</button>
                        </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div id="myTabContent">
                {{-- B All --}}
                <div class="hidden px-4 py-10 seconder-bg" id="all">
                    {{-- B row --}}
                ...
                {{-- B Categories --}}
                @foreach ($recent_plates['categories'] as $item)
                    <div class="hidden p-4 py-10 seconder-bg" id="{{ $item->name }}"">
                        <div class="grid justify-center gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
                            @foreach ($recent_plates['plates'] as $card)
